...
import {
  ...
  Calendar, TrendingUp, Radio, Trash2, Copy, ExternalLink,
} from "lucide-react";
...
type FaqStats = {
  ...
  total_searches_home: number;
  total_searches_painel: number;
};

export default function FaqAnalyticsPage() {
  const [logs, setLogs] = useState<SearchLog[]>([]);
  const [faqStats, setFaqStats] = useState<FaqStats>({ total_searches: 0, total_searches_home: 0, total_searches_painel: 0 });
  ...
  const loadData = useCallback(async () => {
    ...
    if (statsResult.data) {
      const statsMap: FaqStats = { total_searches: 0, total_searches_home: 0, total_searches_painel: 0 };
      statsResult.data.forEach(row => {
        const k = row.key;
        const v = Number(row.value);
        if (k === 'total_searches') statsMap.total_searches = v;
        else if (k === 'total_searches_home') statsMap.total_searches_home = v;
        else if (k === 'total_searches_painel') statsMap.total_searches_painel = v;
      });
      setFaqStats(statsMap);
    }
  ...
  useEffect(() => {
    ...
    const logsChannel = supabase
      .channel("faq-analytics-logs")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "faq_search_analytics" },
        (payload) => {
    ...
    const statsChannel = supabase
      .channel("faq-analytics-stats")
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "faq_stats" },
        (payload) => {
          const updated = payload.new as { key: string; value: number };
          setFaqStats(prev => {
            const next = { ...prev };
            const k = updated.key;
            const v = Number(updated.value);
            if (k === 'total_searches') next.total_searches = v;
            else if (k === 'total_searches_home') next.total_searches_home = v;
            else if (k === 'total_searches_painel') next.total_searches_painel = v;
            return next;
          });
        }
      )
  ...
  const handleReset = async () => {
    ...
    setIsResetting(true);
    try {
      const { error } = await supabase.rpc("reset_faq_analytics");
      if (error) {
        toast.error("Erro ao resetar dados", { description: error.message });
      } else {
        setLogs([]);
        setFaqStats({ total_searches: 0, total_searches_home: 0, total_searches_painel: 0 });
        setShowResetModal(false);
        setResetConfirmText("");
        toast.success("Dados resetados com sucesso", {
          description: "Todos os logs e contadores foram zerados.",
        });
      }
    } catch {
  ...
  // Copy term to clipboard and navigate to FAQ editor
  ...
  // Real total searches (correct denominator) considering active filter
  const totalSearchesReal =
    filterPage === "home" ? faqStats.total_searches_home :
    filterPage === "painel" ? faqStats.total_searches_painel :
    faqStats.total_searches;

  const failedCount = filteredLogs.length;
  const totalForRate = totalSearchesReal + failedCount;
          ...
          <div className="shrink-0 flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="painel">Painel de Ajuda</option>
              </select>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/30 p-5 rounded-2xl flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-on-surface">
                  {isLoading ? "..." : (totalSearchesReal + failedCount).toLocaleString("pt-BR")}
                </h3>
              </div>
            ...
            <p className="text-xs text-outline mt-4">
              {totalSearchesReal.toLocaleString("pt-BR")} com resultado · {failedCount} sem resultado
            </p>
          </div>
          ...
          <div className="bg-success/5 border border-success/20 p-5 rounded-2xl flex flex-col justify-between">
            ...
            <p className="text-xs text-success/80 mt-4">Clientes que encontraram resposta no FAQ.</p>
          </div>
          ...
          <div className="bg-warning-light border border-warning/20 p-5 rounded-2xl flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-semibold text-warning-dark mb-1">Maior Gap</p>
                  ...
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-outline">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs">O FAQ parece cobrir todas as buscas recentes para este filtro.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    sortedGaps.map((item, index) => (
                      <tr key={index} className="hover:bg-surface-container-low/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-xs font-mono text-outline">{item.origin}</td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              onClick={() => handleAddFaq(item.term)}
                  ...
                  {isLoading ? (
                    ...
                    filteredLogs.map(log => (
                      <tr key={log.id} className="hover:bg-surface-container-low/50 transition-colors">
                        ...
                        <td className="px-6 py-3 text-xs font-mono text-outline">{log.page}</td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-on-surface">Resetar Analytics</h2>
            ...
            <div className="bg-error/5 border border-error/20 rounded-2xl p-4 text-sm text-error space-y-2">
              ...
              <ul className="list-disc list-inside text-xs space-y-1 text-error/90">
                <li>Todos os logs de buscas com falha ({logs.length} registros)</li>
                <li>Contadores de buscas com sucesso ({faqStats.total_searches})</li>
